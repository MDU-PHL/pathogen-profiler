...
class bam:
    ...
    def call_variants(self,ref_file,caller,bed_file=None,threads=1,calling_params=None,remove_missing=False, samclip=False,min_dp=10):
        ...
        if self.platform == "nanopore" and self.caller=="bcftools":
            ...
            self.calling_params = calling_params if calling_params else ""
            pre_fb = '%(windows_cmd)s | parallel -j %(threads)s --col-sep " " "samtools view -T %(ref_file)s -h %(bam_file)s {1} %(samclip_cmd)s | samtools view -b > %(prefix)s.{2}.tmp.bam && samtools index %(prefix)s.{2}.tmp.bam"' % vars(self)
            fb = '%(windows_cmd)s | parallel -j %(threads)s --col-sep " " "freebayes -f %(ref_file)s -r {1} --haplotype-length -1 %(calling_params)s %(prefix)s.{2}.tmp.bam > %(prefix)s.{2}.tmp.vcf"' % vars(self)
            bcf_view = '%(windows_cmd)s | parallel -j %(threads)s --col-sep " " "bcftools view -c 1 %(prefix)s.{2}.tmp.vcf > %(prefix)s.{2}.view.vcf.gz"' % vars(self)
            bcf_norm = '%(windows_cmd)s | parallel -j %(threads)s --col-sep " " "bcftools norm -f %(ref_file)s %(prefix)s.{2}.view.vcf.gz > %(prefix)s.{2}.norm.vcf.gz"' % vars(self)
            bcf_filter = '%(windows_cmd)s | parallel -j %(threads)s --col-sep " " "bcftools filter -t {1} -e \'FMT/DP<%(min_dp)s\' %(missing_cmd)s -Oz -o %(prefix)s.{2}.vcf.gz %(prefix)s.{2}.norm.vcf.gz"' % vars(self)
            self.calling_cmd =  [pre_fb,fb,bcf_view,bcf_norm,bcf_filter]
